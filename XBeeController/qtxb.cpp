...
void QTXB::send(RemoteCommandRequest *req)
{
    req->assemblePacket();
    if(xbeeFound && serial->isOpen())
    {
        qDebug() << "Transmit: " << req->getPacket().toHex();
        serial->write(req->getPacket());
...
void QTXB::unicastRemoteCommandRequest(QByteArray address, QString cmd, QByteArray param){
    RemoteCommandRequest *req = new RemoteCommandRequest(this);
    req->setDestAddr64(address);
    req->setATCommand(cmd);
    req->setParameter(param);
    QByteArray opt;
    opt.append(2);
    req->setRemoteCmdOptions(opt);
